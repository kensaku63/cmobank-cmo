...
- 次回: 残るセクションはhero.md(別セッション)。hero.md側からも今後、常駐エージェントの気配表示(FV開始直後から右下固定の小さな円形アバター、character system)の見た目について申し送りが来る可能性がある。agent-dialogue.md第6節がその前提(形状・位置)を既に規定している
...
- 次に書くべきセクション企画: agent-dialogue.md(対話UIの見た目・起動後の会話設計)。着手時はhero.md(常駐エージェントの気配の見た目)、flagship-story.md(story映像終端からの引き渡し演出とカメラワーク制約)、closing-cta.md(常時表示CTAとのドッキング位置調整)の3本を必読にし、同一キャラクターの視覚的一貫性を引き継ぐこと
