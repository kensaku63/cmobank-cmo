...
- サイト全体の情報設計を site-design.md に確定(4セクション: FV / story映像 / セールスエージェント対話 / トライアルCTA)。詳細企画はセクションごとにfresh sessionへ分割済み
...
- 重要な申し送り: story映像の最後のカメラワーク(引き→対話面の起動)と、site-design.mdの起動トリガー(視聴完了 or セクション下端到達/滞在時間)を一致させる制約は、対話面の起動アニメーション設計を拘束する。agent-dialogue.mdセッションが別の起動演出をすでに決めていた場合、映像側の設計を優先するようkensakuが転送済み
